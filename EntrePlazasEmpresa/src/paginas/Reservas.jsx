...
export default function Reservas({ negocioId, token }) {
  ...
  const esPasado = (fecha) =>
    new Date(fecha) < new Date(new Date().toISOString().split("T")[0]);
    ...
    <div className="reservas-root">
      ...
      {diasOrdenados.length === 0 ? (
        <div className="vacio">
          ...
          ></i>
          No hay reservas{" "}
          {filtro !== "TODAS" ? `con estado ${filtro.toLowerCase()}` : ""}
        </div>
      ) : (
        <div className="dias-lista">
          {diasOrdenados.map((dia) => {
            ...
            const pendientes = reservasDelDia.filter(
            ...
            return (
              <div
                key={dia}
                className={`dia-grupo ${pasado ? "pasado" : ""} ${esHoy(dia) ? "hoy" : ""}`}
              >
                <button
                ...
                >
                  <div className="dia-cabecera-izq">
                    ...
                    <span className="dia-fecha-corta">
                  ...
                  <div className="dia-cabecera-der">
                    ...
                    {pendientes > 0 && (
                      <span className="dia-pendientes">
                        {pendientes} pendiente{pendientes > 1 ? "s" : ""}
                      </span>
                    )}
                    <i
                      className={`bi bi-chevron-${expandido ? "up" : "down"} dia-chevron`}
                    ></i>
                  </div>
                </button>
                  ...
                  <div className="dia-reservas">
                    ...
                    {reservasDelDia.map((r) => (
                      <div key={r.id} className="tabla-fila">
                        ...
                        <span>
                          <span
                            className={`badge-estado ${r.estado.toLowerCase()}`}
                          >
                            {r.estado.charAt(0) +
                              r.estado.slice(1).toLowerCase()}
                          </span>
                        </span>
